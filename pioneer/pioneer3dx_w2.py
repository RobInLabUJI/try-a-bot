...
def init():
    ...
    global _enc_pbl
    _enc_pbl = _rospy.Publisher(_controllerName+'/encoder_updated',_Empty,queue_size=100)
...
def _enableSonar():
        ...
        global _TIME_STEP
        global _so_pbl
        _so_pbl = _rospy.Publisher(_controllerName+'/sonar_updated',_Empty,_Empty,queue_size=100)
        #for s in range(16):
        for s in range(8):
            deviceName = 'so'+str(s)
            sp = _rospy.ServiceProxy(_controllerName+'/'+deviceName+'/enable',_sensor_enable)
